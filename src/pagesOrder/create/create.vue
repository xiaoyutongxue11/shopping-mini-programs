...
const onOrderSubmit = async () => {
  ...
  const { result } = await postMemberOrderAPI({
    addressId: orderAddress.value.id,
...
<template>
  <scroll-view scroll-y class="viewport" style="padding-bottom: 120rpx">
    ...
    >
      <view class="user"> {{ orderAddress?.receiver }} {{ orderAddress?.contact }} </view>
      <view class="address"> {{ orderAddress?.fullLocation }} {{ orderAddress?.address }} </view>
      ...
      >
        ...
        <view class="meta">
          ...
          <view class="prices">
            ...
            <view class="price symbol" v-if="item.payPrice !== item.price">{{ item.price }}</view>
          </view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !orderAddress }" @tap="onOrderSubmit"> 提交订单 </view>
  </view>
</template>
